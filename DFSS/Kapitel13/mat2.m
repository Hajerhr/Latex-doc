...
for rf = 10:10:90
    for t = 10:10:90
        xneu = [xneu; [1 rf t rf*t rf$\mathrm{\wedge}$2 t$\mathrm{\wedge}$2]];
    end;
end; 
...
for n = 1:length(xneu)
    x0 = xneu(n,:)';
    CpPmin = [CpPmin bmat'*x0 + ...
        tinv((1-gamma)/2,FG)*sqrt(Cpres'*Cpres/FG)*sqrt(1+x0'*inv(X'*X)*x0)];
    CpPmax = [CpPmax bmat'*x0 + ...
tinv((1+gamma)/2,FG)*sqrt(Cpres'*Cpres/FG)*sqrt(1+x0'*inv(X'*X)*x0)];
end;